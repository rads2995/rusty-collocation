pub(crate) mod ipopt {
    use crate::bindings::ipopt::{ipindex, ipnumber};

    pub fn objective_function(x: &[ipnumber]) -> ipnumber {
        x[0] * x[3] * (x[0] + x[1] + x[2]) + x[2]
    }

    pub fn constraint_functions(x: &[ipnumber], g: &mut [ipnumber]) {
        g[0] = x[0] * x[1] * x[2] * x[3];
        g[1] = x[0] * x[0] + x[1] * x[1] + x[2] * x[2] + x[3] * x[3];
    }

    pub fn gradient_objective_function(x: &[ipnumber], grad_f: &mut [ipnumber]) {
    ...
    pub fn jacobian_constraint_elements(i_row: &mut [ipindex], j_col: &mut [ipindex]) {
        i_row[0] = 0;
        j_col[0] = 0;
        i_row[1] = 0;
        j_col[1] = 1;
    ...
    pub fn jacobian_constraint_function(x: &[ipnumber], jac_g: &mut [ipnumber]) {
        jac_g[0] = x[1] * x[2] * x[3];
        jac_g[1] = x[0] * x[2] * x[3];
        jac_g[2] = x[0] * x[1] * x[3];
        jac_g[3] = x[0] * x[1] * x[2];
    ...
    pub fn hessian_elements(i_row: &mut [ipindex], j_col: &mut [ipindex]) {
        i_row[0] = 0;
        j_col[0] = 0;
        i_row[1] = 1;
        j_col[1] = 0;
